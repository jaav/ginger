...
public class Activities extends GingerController {
	...
	public static void copy(Long id) {
		...
		try {
			...
			copy.pers = entity.pers;
			copy.indicatoren = entity.indicatoren;
  ...
  if (request.params.get("sector_999") != null) {
    ...
    if(hasSubSecs){
      for (String id : ids) {
        ...
        idswhere.append("saj").append(counter).append(".sectorId = ").append(id).append(") or ");
        counter++;
      }
    ...
    if(!emptyParents.isEmpty()){
      ...
      for (Long parentSector : emptyParents) {
        ...
        idswhere.append("(sajsid").append(counter).append(".ouder = ").append(parentSector).append(" or sajsid").append(counter).append(".id = ").append(parentSector).append(")) or ");
        counter++;
      }
    }
	...
	private static void getActivityBySector(ArrayList<String> whereClause, StringBuffer joinClause) {
		if (request.params.get("sector_999") != null) {
			...
			//Create a query like this "... join SectorActivityJunction as SAJ on A.id=SAJ.`activityId_id` where `sectorId_id` in (2, 3, 4, 5, 6, 7);"

			joinClause.append(" join act.sectorActivityJunctions saj").append(" ");
			if (hasSubSecs) {
				idswhere.append(" (saj.activityId=act.id and saj.sectorId in (");
				for (String id : ids) {
					idswhere.append(id).append(",");
				}
			}
			if (!emptyParents.isEmpty()) {
				for (Long parentSector : emptyParents) {
					idswhere.append(parentSector);
					List<Sectors> subsecs = models.Sectors.find("ouder = "+parentSector).fetch();
					if (subsecs != null) {
						for (Sectors subsec : subsecs) {
							idswhere.append(subsec.getId()).append(", ");
						}
					}
				}
			}
			...
			if (idswhere.length() > 3) whereClause.add(idswhere.deleteCharAt(idswhere.length()-2).append(")) ").toString());
		}
	}
	...
	private static void getActivityByItemsUsed(ArrayList<String> whereClause, StringBuffer joinCaluse) {
		...
		if (hasItems) {
			...
			for (Long id : ids) {
				...
				if (counter > 1)
					idswhere.append(" iia").append(counter - 1).append(".activityId=iia").append(counter).append(".activityId and ");
